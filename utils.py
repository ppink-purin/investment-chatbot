...
def generate_google_query(user_input):
    prompt = f"""
    아래 질문 내용을 바탕으로 ETF 종목 탐색에 적합한 구글 검색에 유리한 형태의 명료한 검색 질의어로 변경해줘.
    구글 검색의 특성에 맞게 구체적인 키워드 단위로 추출되는게 필요하고, 쉼표로 연결되는 여러 문장의 조합보다는 한 문장 형태의 명료한 키워드 조합으로 질의어를 완성해줘.
    전체 문장을 따옴표로 감싸게 되면 문장 전체가 일치하는 검색결과만 나오게 되니 절대 감싸지 말아줘:
...
def generate_gpt_response(question, scraped_text, etf_count=3):
    ...
    위 정보를 기반으로 고객 질문에 600자 이내의 간결한 답변을 해줘. 만약 구글 검색 정보가 한국어로 읽히지 않는 경우, 적합한 한국어 인코딩 결과를 적용해줘.
